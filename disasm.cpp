...
void disasmaddr(uint16_t m, uint32_t a) {
    if (m & 7) {
        ...
        case 027:
            a += 2;
            Serial.printf("$%06o", cpu.read16(a));
            return;
        case 037:
            a += 2;
            Serial.printf("*%06o", cpu.read16(a));
        ...
        case 067:
            a += 2;
            Serial.printf("*%06o", (a + 2 + (cpu.read16(a))) & 0xFFFF);
            return;
        case 077:
            Serial.printf("**%06o", (a + 2 + (cpu.read16(a))) & 0xFFFF);
            return;
    ...
    case 060:
        a += 2;
        Serial.printf("%06o (%s)", cpu.read16(a), rs[m & 7]);
        break;
    case 070:
        a += 2;
        Serial.printf("*%06o (%s)", cpu.read16(a), rs[m & 7]);
...
void disasm(uint32_t a) {
    const uint16_t ins = cpu.read16(a);

    D l = {0, 0, "", 0, false};
    for (uint16_t i = 0; disamtable[i].ins; i++) {
        l = disamtable[i];
        if ((ins & l.mask) == l.ins) {
            break;
        }
    ...
    Serial.printf("%s", l.msg);
    if (l.b && (ins & 0100000)) {
        Serial.printf("B");
    }
    const uint16_t s = (ins & 07700) >> 6;
    const uint16_t d = ins & 077;
    uint16_t o = ins & 0377;
    switch (l.flag) {
    case S | DD:
        Serial.printf(" ");
    ...
    case DD:
        ...
        break;
    case RR | O:
        Serial.printf(" %s,", rs[(ins & 0700) >> 6]);
        o &= 077;
        [[fallthrough]];
    case O:
        if (o & 0x80) {
            Serial.printf(" -%03o", (2 * ((0xFF ^ o) + 1)));
        } else {
            Serial.printf(" +%03o", (2 * o));
        };
        break;
    case RR | DD:
        Serial.printf(" %s, ", rs[(ins & 0700) >> 6]);
